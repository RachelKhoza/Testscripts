...
def lambda_handler(event, context):
    ...
    snapshot_identifier = f"lambda-rds-snapshot-{db_instance_id}-{datetime.datetime.now().strftime('%Y%m%d%H%M')}"
    ...
    try:
        ...
        # Iterate through snapshots and delete those older than 7 days, created by this Lambda function
        for snapshot in snapshots:
            ...
            if snapshot_create_time < cutoff_date and snapshot_id.startswith(f"lambda-rds-snapshot-{db_instance_id}-"):
